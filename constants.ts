...
const AVATAR_URL = "https://xland.eu.org/autoupload/fr/UniTf9v-0dh7Y_2n8qm7RYRDL2MdoaCZmDovFomx91Oyl5f0KlZfm6UsKj-HyTuv/20251130/1nVa/1280X1707/ca9f0f1d6a108caa7f8a9995820d39b1.jpg";
        ...
        projects: [
          {
            ...
            responsibilities: [
              "数据分析：基于 T-7 日用户粒度数据，对 30 余项关键行为指标进行动作率、LT7 及优质内容浓度的 spearman 相关性分析，结合用户行为渗透情况，筛选出生态允许且影响显著的目标，为后续建模与参数调优提供数据支撑。",
              "策略设计与参数调优：在明确优势目标后，主导精排环节的参数调优工作。对已有埋点的行为，通过分析目标分值分布、方差及模型分歧胜率，确定合理参数并推动实验上线，持续跟踪效果并进行迭代优化。",
              "实验评估与效果分析：负责精排新增特征（如横屏、进度条）的实验观测与效果验证。AB 结果显示，大盘用户 LT7 提升 0.3%，30 秒内手搜场景下 LT7 提升 0.8%。"
            ],
